...
import { createInitialState, ensureAssignment, localDateFor, orderedHistory, questionFor, saveAnswer, selectQuestion } from './state';
...
describe('deterministic local daily journey', () => {
  ...
  test('a complete preview cycle contains no duplicate question', () => {
    const ids = QUESTIONS.map((_, index) => selectQuestion({ profileId: 'user-a', localDate: `2026-09-${String(index + 1).padStart(2, '0')}` }).id);
  ...
  test('draft is removed only after durable answer state is created', () => {
  ...
  test('history is newest first and keeps question snapshots', () => {
